...
@Configuration
public class BatchConfigurationSaveFileToDb {
    ...
    @Bean
    public Job saveDbToFile(Step step2, JobExecutionListener listener) {
        return jobBuilderFactory.get("saveDbToFile").incrementer(new RunIdIncrementer()).listener(listener).start(step2)
                .build();
    }
